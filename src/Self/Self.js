...
const Self = () => {
    ...
    const SmallBtn = (item) => (
        ...
        <div className={`keyBtn KeyBtn-${pressed.toUpperCase() === item.id.toUpperCase() ? 'pressed' : ''}`} key={item.id} onClick={() => onKeyClick(item)}>
    ...
    const MediumBtn = (item) => (
        ...
        <div className={`keyBtn keyBtn-${item.size}  ${item.size === "xxl" && shiftPressed ? 'shifted' : ''} KeyBtn-${pressed === item.id ? 'pressed' : ''}`} key={item.id} onClick={() => onSpecialKeyClick(item)}>
    ...
    return (
        ...
        <div className={`font-${fonts[fontType]}`} style={{
            display: 'flex', minHeight: "90vh", margin: "10px 20px", justifyContent: 'space-between', alignItems: 'center', flexDirection: 'column'
        }}>
            {/* <input /> */}
            ...
            <div>
                {/* <p>{pressed}</p> */}
                <div>
                    <select value={fontType} onChange={onFontClick}>
                        {
                            fonts.map((item, index) => <option key={index} value={index}>
                                {item.toUpperCase()}
                            </option>)
                        }
                    </select>
                ...
                <div style={{ width: "100vw", textAlign: 'center' }}>
                    ...
                    <textarea className={`font-${fonts[fontType]}`} name="Text1" rows="5" id="one" onKeyDown={handleKeyPress} autoFocus value={inputText} style={{ minWidth: "80%", border: 'none', }}></textarea>
